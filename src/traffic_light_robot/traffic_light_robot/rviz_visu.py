...
class RVizVisualizer(Node):
    ...
    def publish_markers(self):
        ...
        # Traffic light state indicator (sphere)
        state_marker = Marker()
        ...
        speed_bar.header.stamp = self.get_clock().now().to_msg()
        ...
        max_speed = 0.6
        speed_height = (self.current_speed / max_speed) * 2.0
        speed_bar.pose.position.z = speed_height / 2.0
        speed_bar.scale.x = 0.2
        speed_bar.scale.y = 0.2
        speed_bar.scale.z = speed_height if speed_height > 0.01 else 0.01
        ...
        p1 = Point(x=1.3, y=-1.0, z=target_height)
        p2 = Point(x=1.7, y=-1.0, z=target_height)
        target_line.points = [p1, p2]
        ...
        if len(self.red_history) > 0:
            red_val = list(self.red_history)[-1] / 5000.0
            green_val = list(self.green_history)[-1] / 5000.0
            yellow_val = list(self.yellow_history)[-1] / 5000.0
            ...
            for idx, (val, color, y_offset) in enumerate(colors_data):
                ...
                bar.pose.position.z = min(val, 1.0) / 2.0
                bar.scale.x = 0.15
                bar.scale.y = 0.15
                bar.scale.z = min(val, 1.0)
                bar.color = color
